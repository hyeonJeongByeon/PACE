// Pre-visit survey items — HANDOVER_SPEC_V2.md §3, provenance tags preserved.
// §3.6 format rules: structured options for recall items; free text only where
// the participant's own words matter, each with a memory-cue line.
// Researcher-editable.
...
window.PACE_SURVEY = {
  ...
  partA: [
    { key: 'problem_text', type: 'text', required: true, source: '[GR item 1]',
      ...
      cue: 'Cues: when it started · how bad it gets · what it stops you from doing',
    ...
    { key: 'suspected_cause', type: 'text', required: false, source: '[GR item 9]',
      ...
      cue: 'Cues: something you read · something a friend or family member said · a past experience',
      placeholder: '' },
    { key: 'visit_goal', type: 'text', required: true, source: '[PACE-GS]',
      ...
      cue: 'Cues: explain what\'s going on · change the plan · order a test · just listen',
      placeholder: '' },
    { key: 'must_ask_question', type: 'text', required: true, source: '[GR item 10, PACE-GS]',
      ...
      cue: 'Cues: about the cause · about a treatment · about what happens next',
      placeholder: '' },
  ],

  // §3.3 item 7 — structured treatment list [NEW, pilot F5]. Free-text names
  // are fine ("the oral one my dermatologist gave me" is valid).
  currentTreatments: {
    key: 'current_treatments', source: '[NEW]',
    label: 'Medications or treatments you are using — or have tried — for this problem (optional)',
    sub: 'You don\'t need exact names. "The cream from the pharmacy" works.',
    statusOptions: ['Currently using', 'Tried in the past'],
    helpedOptions: ['Helped', 'Didn\'t help', 'Made things worse', 'Not sure'],
  },

  // Part B — component screener [LEAPS]. Never / Sometimes / Often.
  screener: [
  ...
  preferredFocus: {
    ...
    // §3.4: tie-breaker + research variable ONLY — never the assignment driver.
    options: [
      { value: 'P', label: 'Telling my story clearly and completely' },
      { value: 'A', label: 'Getting my questions answered' },
      { value: 'C', label: 'Making sure I understood what the provider said' },
      { value: 'E', label: 'Saying what\'s worrying me' },
    ],
  },

  // §3.5 safety item — verbatim hard constraint in the persona prompt.
  contentExclusions: {
